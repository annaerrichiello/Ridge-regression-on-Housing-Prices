...
y_test_housepc <- matrix(c(median_house_value), ncol = 1, nrow=4)

alphas <- matrix(seq(3, -2, by = -.1), ncol = 3, nrow = 3)
I <- diag(3)
pred_housepc <- x_test_housepc%*%(((t(x_test_housepc)%*%x_test_housepc+alphas%*%I))^(-1))%*%t(x_test_housepc)%*%y_test_housepc
show(pred_housepc_)
...
evaluations <- function (true, predicted, df) {
  ...
  sst <- sum((pred_housepc_ - mean(y_test_housepc))^2)
  rsq <- 1 - sse / sst
